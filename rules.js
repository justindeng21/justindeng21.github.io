...
function begin_play()
{
	...
	if(player1.value == "LeBron is the greatest")
		alert("You damn right he is")

	if(isEmpty(player1.value) || isEmpty(player2.value))
	{
		alert("Two Player game, both fields must be completed")
		return
	}
...
function reset_play()
{
		...
		started = false
		player1.value = "", player1.disabled = false
		player2.value = "", player2.disabled = false
		if(whose_move == 0)
			toggle_move()
		for(i = 0; i < table_ids.length; i++)
		{
			var lmao = document.getElementById(table_ids[i])
			lmao.innerHTML = table_ids[i]
		}
}
...
function play() 
{
	...
	var x = "X"
	var o = "O"
	if(!move.value.match(regEx))
	{
		$("#xcx").modal('show')
		return
	}
	var element = document.getElementById(move.value)
	...
	else
	{
		...
		if(whose_move() == 1)
		{
			element.innerHTML = "X"
			verify(move.value)
			turnInfo.innerHTML = "Turn for : " + o.bold()
		}
